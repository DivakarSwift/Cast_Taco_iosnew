...
protocol ComedyViewControllerDelegate {
...
class ComedyViewController: UIViewController {
    ...
    var delegate: ComedyViewControllerDelegate?
    ...
    func reloadHeader() {
        ...
        for bet in usersOnBet {
            if bet.category == BetType.Comedy {
    ...
    @objc func unhideContainer(sender:UIGestureRecognizer) {
        ...
        embeddedVC.selectedCategoryType = BetType.Comedy
...
//    }
    ...
    private func setBet(_ forRank: Int) {
        Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Comedy, forRank)
    }
    
    
    private func removeBet ( _ forRank: Int) {
        Singleton.shared.removeBet(.Comedy, forRank: forRank)
    }
    
    private func isUserOnBet(_ forRank:Int) ->Bool {
        return Singleton.shared.isUserOnBet(.Comedy, forRank: forRank)
    }
    
}

extension ComedyViewController :UITableViewDataSource, UITableViewDelegate {
...
extension ComedyViewController : SelectPicViewControllerDelegate {
    func openUserProfile() {
         delegate?.OpenUserProfile(viewModel.selectedUser)
    }
    
    func rankSelected(rank: Int, uid: String?) {
        Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Comedy, rank)
        reloadHeader()
    }
    
    func rankRemoved(rank: Int, uid: String?) {
        Singleton.shared.removeBet(.Comedy, forRank: rank)
        reloadHeader()
    }
...
extension ComedyViewController : setNominieesViewDelegate {
    func firstCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(1) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Comedy, 1)
        } else {
            Singleton.shared.removeBet(.Comedy, forRank: 1)
        }
        self.reloadHeader()
    }
    ...
    func secondCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(2) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Comedy, 2)
        } else {
            Singleton.shared.removeBet(.Comedy, forRank: 2)
        }
        self.reloadHeader()
    }
    ...
    func thirdCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(3) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Comedy, 3)
        } else {
            Singleton.shared.removeBet(.Comedy, forRank: 3)
        }
        self.reloadHeader()
    }
...
extension ComedyViewController : LeaderBoardSegmentsViewModelDelegate {
    func changePlayerWindowAppearance(shouldHide: Bool, user: User?) {
