...
class SoundViewController: UIViewController {
    ...
    @objc func unhideContainer(sender:UIGestureRecognizer) {
        ...
        embeddedVC.selectedCategoryType = BetType.Sound
...
//    }
    ...
    private func setBet(_ forRank: Int) {
        Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Sound, forRank)
    }
    
    private func isUserOnBet(_ forRank:Int) ->Bool {
        return Singleton.shared.isUserOnBet(.Sound, forRank: forRank)
    }
...
extension SoundViewController : SelectPicViewControllerDelegate {
    func openUserProfile() {
         delegate?.OpenUserProfile(viewModel.selectedUser)
    }
    
    func rankSelected(rank: Int, uid: String?) {
        Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Sound, rank)
        reloadHeader()
    }
    
    func rankRemoved(rank: Int, uid: String?) {
        Singleton.shared.removeBet(.Sound, forRank: rank)
        reloadHeader()
    }
...
extension SoundViewController : setNominieesViewDelegate {
    func firstCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(1) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Sound, 1)
        } else {
            Singleton.shared.removeBet(.Sound, forRank: 1)
        }
        self.reloadHeader()
    }
    ...
    func secondCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(2) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Sound, 2)
        } else {
            Singleton.shared.removeBet(.Sound, forRank: 2)
        }
        self.reloadHeader()
    }
    ...
    func thirdCirlceTapped() {
        if viewModel.showPlayer, !isUserOnBet(3) {
            Singleton.shared.setBet(viewModel.selectedUser, forCatgory: .Sound, 3)
        } else {
            Singleton.shared.removeBet(.Sound, forRank: 3)
        }
        self.reloadHeader()
    }
...
extension SoundViewController : LeaderBoardSegmentsViewModelDelegate {
    func changePlayerWindowAppearance(shouldHide: Bool, user: User?) {
